...
仕様版: 1.0。更新日: 2026-09-20。本書1ファイルを実装基準とする。リポジトリの従来の統合仕様、先行する設計改善案、日本語IMEを必須とする追加要件を統合した。Cloudflare Access、特定メールドメイン制限、Worker側の本文復号、R2 SSE-Cを主たる暗号化とする旧案は採用しない。
...
初期期限は絶対30日・無操作7日。失効とaccountのauth_epochをサーバーで確認する。機密操作は5分以内のstep-up再認証を要求する。セッション期限と端末内の復号鍵保持期限は別である。
...
WebのVaultKeyは解除中メモリーだけに置く。再読み込み後はパスキーまたは復旧キーで再解除する。IndexedDBには暗号化キャッシュと最新未同期ドラフトだけを置き、平文や鍵をlocalStorage/sessionStorageへ置かない。
...
非表示/バックグラウンド移行から5分で再解除を要求する。タイマー停止を前提に復帰時の時刻で判定する。明示ロック・OSから通知された保護データ利用不可では即座に保護する。WebはOSロックを常に検出できると仮定しない。
...
空文書は空textブロック1個。先頭・末尾・隣接メディアの間には空でも編集可能なtextブロックを確保する。通常の文字編集でIDを再生成しない。メディア挿入でtextを分割するときは左側のIDを維持し右側を新規発行、統合するときは左側IDを維持する。
